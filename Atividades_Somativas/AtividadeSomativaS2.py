    ...
    if len(lista) == 0:
        ...
        for dicionario_geral in lista:
            if lista == lista_estudantes:
                ...
                print(dicionario_geral["codigo_turma"],"|", dicionario_geral["codigo_professor"],"|", dicionario_geral["codigo_disciplina"])
                print()

            else:
                print("\nCOD. TURMA | COD. ESTUDANTE")
                print(dicionario_geral["codigo_turma"],"|", dicionario_geral["codigo_estudante"])
                print()


def atualizar(codigo_para_editar, nome_arquivo):
    codigo_para_editar = None
    for estudante in lista_estudantes:
        if estudante["codigo"] == codigo_para_editar:
            ...
            print(f"O estudante", estudante["codigo"], "foi atualizado com sucesso!")
    ...
    for disciplina in lista_disciplinas:
        if disciplina["codigo"] == codigo_para_editar:
            ...
            print(f"A disciplina", disciplina["codigo"], "foi atualizada com sucesso!")
    ...
    for professor in lista_professores:
        if professor["codigo"] == codigo_para_editar:
            ...
            print(f"O professor", professor["codigo"], "foi atualizado com sucesso!")
    ...
    for turma in lista_turmas:
        if turma["codigo"] == codigo_para_editar:
            ...
            print(f"A turma", turma["codigo"], "foi atualizada com sucesso!")
            salvar_arquivo(nome_arquivo, lista_turmas)

    for matricula in lista_matriculas:
        if matricula["codigo"] == codigo_para_editar:
            matricula["código_estudante"] = input("Digite o novo código: ")
            print(f"A matrícula", matricula["codigo"], "foi atualizada com sucesso!")
            salvar_arquivo(nome_arquivo, lista_matriculas)


def excluir(codigo, lista, nome_arquivo):
    excluir_da_lista = None
    for dicionario_geral in lista:
        if dicionario_geral["codigo"] == codigo:
            excluir_da_lista = dicionario_geral
            break

    if excluir_da_lista is None:
        print("Código não encontrado")

    else:
        lista.remove(excluir_da_lista)
...
def main():
    ...
    carregar_arquivo("lista_estudantes.json")
    carregar_arquivo("lista_disciplinas.json")
    carregar_arquivo("lista_professores.json")
    carregar_arquivo("lista_turmas.json")
    carregar_arquivo("lista_matriculas.json")
        ...
        if opcaomenu < 6 :
            ...
            while True:
                ...
                if operacaomenu2 == 1:
                    ...
                    print(escolha_menu_principal,"cadastrado com sucesso!")
